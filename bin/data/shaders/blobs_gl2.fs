...
void main( void ) {
   ...
   vec2 tp = (gl_TexCoord[0].xy/resolution.x) * 2.0 - vec2(1.0,resolution.y/resolution.x);
   ...
   vec4 vidColor = vec4(texture2DRect(videoTex, gl_TexCoord[0].xy * videoScaling).rgb, 1.0);
   //fragColor = mix( vidColor, blobColor, interp);

   //make grayScale
   float grayComp = (vidColor.r + vidColor.g + vidColor.b) / 3;
...
gl_FragColor = mix( vidGray, blobColor, interp);
   //gl_FragColor = mix( vidColor, blobColor, interp);
